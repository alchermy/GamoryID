...
<section class="card" aria-labelledby="shop-topups-title">
    ...
    <div class="table-wrap"><table><thead><tr><th scope="col">วันที่ส่ง</th><th scope="col">ผู้ส่ง</th><th scope="col">เครดิต</th><th scope="col">สถานะ</th><th scope="col">เหตุผล / หมายเหตุ</th><th scope="col">สลิป</th></tr></thead><tbody>
        @forelse($topUps as $topUp)
            <tr @class(['flagged' => $topUp->auto_check_failed])><td>{{ $topUp->created_at->timezone('Asia/Bangkok')->format('d/m/Y H:i') }}</td><td>{{ $topUp->submittedBy?->name ?? '—' }}</td><td class="credit">{{ number_format($topUp->credit_amount) }}</td><td><span class="status {{ $topUp->status }}">{{ $statusLabels[$topUp->status] ?? $topUp->status }}</span>@if($topUp->auto_check_failed)<span class="flag-badge" title="ตรวจสลิปอัตโนมัติไม่ทำงาน">⚠</span>@endif</td><td class="muted">{{ $topUp->review_note ?: 'รอตรวจสอบรายการ' }}</td><td><a class="slip-link" target="_blank" rel="noopener" href="{{ route('admin.top-ups.slip', $topUp) }}">เปิดสลิป</a></td></tr>
